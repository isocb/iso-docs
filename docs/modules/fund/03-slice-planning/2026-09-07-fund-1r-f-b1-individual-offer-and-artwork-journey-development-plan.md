...
Next human decision/test: five-step local preparation smoke in 2026-09-17-fund-simple-store-preparation-review-and-test.md. Keep VAT A0/A/B PASS. Development PDF is not classroom-ready artwork; public publication/purchase smoke stays paused until release capabilities exist. Separate review must assess/finish the explicitly remaining regression before promotion.
Safe resumption point: demonstrate supported C1 setup through C2 template preparation; use focused checks for unresolved risks rather than repeating the whole remote suite without cause. Preserve first-publication commission locking, pre-opening preparation, finalised evidence and the local test bed. B1 Now / 1R-G planning Next unchanged; no promotion authorised here.
```
