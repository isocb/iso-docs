...
Status: **Chris accepts the C1 operation and notes-to-PDF smoke (1 PASS, 2 PASS), with a separate critical tenant-template editor placement failure. The bounded correction at `fbe8694b03932c310844d7c019bd3394b7809182` moves editing to FUND setup and adds the requested formatting. B1 remains open pending its focused human check; no retained database migration or promotion.**
...
remains High. Historical PASS remains at its original tested boundary; the C1 human
checks are now PASS, with the separate settings-placement correction open. No retained/shared migration or online promotion is authorised by
...
Chris reports the C1/notes smoke **1 PASS, 2 PASS**, with a separate **critical placement
FAIL**: Individual artwork instructions belong to the tenant's template, never to a Project.
Correct the existing B1 candidate now: move the two editors and existing tenant storage
access to FUND setup (`/app/fund/settings`); support H1/H2/H3, body, underline and bold.
Keep one shared Individual template configuration with its accepted portrait/landscape
variants. Projects select a template and retain finalisation/locking; no per-Project wording
override is introduced. Existing confirmed snapshots/PDFs must remain unchanged.

High control is retained for tenant-admin authority. Prove settings without a Project,
C2/foreign/inactive/impersonated denial, strict payloads, safe formatting and fixed-panel
overflow refusal, shared defaults and frozen historical output. Failed saves must leave
settings/audit unchanged; rollback must preserve existing snapshots and compatible reads.
No schema migration is needed. Work stays on the existing isolated candidate and owned
synthetic local target; no retained data, runtime configuration or promotion changes.

The fuller FUND setup **Template Manager** tab (table, row-click CRUD modal, Add new button)
is a future refinement in the existing template brief/parent planning, not implementation
scope now. The root Now/Next pair is unchanged. Stop at verified local correction and the
minimum new settings-to-PDF human check; do not repeat the accepted C1/layout smoke.

## Restart Checkpoint

```text
Current state: C1 operation/notes human checks 1 PASS and 2 PASS retained. Separate critical settings-placement finding corrected locally: tenant-wide rich-text editor on FUND setup, no Project editor, selection/locking retained. Automated correction proof PASS; focused correction human acceptance pending. B1 stays open.
Last proven commit: fbe8694b03932c310844d7c019bd3394b7809182 on feature/fund-accepted-pdf-integration, after 3e82adb1 / e8a3c900 and released c3998084. Candidate committed and published on the existing work branch; still before dev consolidation.
...
Next human decision/test: Two short correction checks in 05: save/reload formatted tenant instructions in FUND setup, then inspect them in the fresh synthetic Project PDF with selection/locking and no Project editor. Do not repeat accepted C1/layout/handwritten smoke.
...
Status: **Bounded local implementation authorised by Chris on 17 September; implemented at `e8a3c900`, automated proof PASS; 18 September PDF checks 1 and 2 PASS; C1 notes-to-PDF human check PASS; separate tenant-settings placement correction open.** This refines the existing B1/1R-F work and coordinates its existing
