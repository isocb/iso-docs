...
Next human decision/test: Chris authorises simple launch preparation, stopping for unresolved implications. Focused review asks whether never-published Projects with no prior commission acceptance or Orders may initialise current terms once after a past start date, then lock immediately. Recommended exception is not approved yet. A0/A/B VAT PASS retained; C/publication smoke paused. Separate review/promotion remain outstanding.
Safe resumption point: resolve the first-setup question in launch plan section 6, then resume authorised preparation on local 6ebaac46. Reuse existing default Product initialisation; no code, schema, fixture or environment changes made at this pause. Preserve existing evidence and test bed; old wf1 stays archived. Real-release/public Store/purchasing are later dependencies; no live approval.
```
