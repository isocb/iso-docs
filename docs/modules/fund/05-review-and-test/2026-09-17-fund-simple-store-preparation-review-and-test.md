...
Status: **Local implementation; bounded automation PASS. Broader regression and separate review open; human smoke not run.**
Control depth: **High**.
...
## Chris's Short Local Smoke — Pending
...
1. **C1 → FUND → FUND setup:** save the intended producer default commission. As the organisation
   ...
   Edit a Product without photography: a tenant logo or neutral placeholder appears automatically.
2. **C1 → Event → Commission:** keep the producer default or save an optional dated ladder.
   ...
   inherited flat rate/complete ladder. No separate commission offer needs creating or accepting.
3. Leave **Remove Products** off on one Project. On a second unfinalised Project enable it,
   ...
   by Store refresh, including when the Project start date is already in the past.
4. As the exact organiser, use an Individual Project whose opening date is in the future but
   ...
   onboarding is required. Re-download preserves the evidence and Product removal is locked.
   This tests preparation; do not distribute this emulator PDF to classrooms.
5. Confirm readiness explains the future opening date and the unfinished Individual release
   ...
   purchase or report an end-to-end Store launch PASS for this increment.
...
The immediate action is the local preparation demonstration. Separate review must address the
...
Staging/environment proof and human acceptance remain pending; main/live is held. Any later
