...
| [`2026-07-15-fund-application-artwork-template-refinement.md`](../01-cr-inputs/2026-07-15-fund-application-artwork-template-refinement.md) | Consumed by accepted `1R-F` parent; source-faithful `1R-F-A-R1B` automation and 12/12 human/physical review pass; Stage B Linux parity and exact `139d09c4` dev scan pass; dedicated Stage C contract accepted and authorised on 2026-08-12; downstream short-code/single-artwork policy preserved in parked `1R-H-A` | Stage C remains root `Now`. Corrected exact job exposed `page.evaluate: ReferenceError: __name is not defined`, identically reproduced locally at `renderer.ts:157`; deterministic runner defect, two jobs/no third, no more attempt. Prove fresh-prefix zero and teardown; retain reviewed local correction/new candidate. External execution/removal remain governed by the accepted assumption-test gate; `1R-H-A` remains inventory only and no production implementation is authorised |
...
```text
ROOT NOW  -> conclude FUND 1R-F-A Stage C external attempt: corrected exact job found Playwright __name browser-serialization defect; prove fresh-prefix zero, teardown, retain local runner correction/new-candidate work
...
`__name` failure. Prove prefix zero, teardown, retain local correction/new-candidate work.
